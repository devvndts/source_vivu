...
class CategoryController extends Controller
{
    ...
    public function ShowItems($response, Request $request)
    {
        ...
        $background_category = '';
        if ($row_detail["level"] == 0) {
            $background_category = $row_detail['photo'];
        } elseif ($row_detail["level"] == 1) {
            $row_detail_parent = $this->categoryRepo->GetOneItem($row_detail['ids_level_1']);
            $background_category = $row_detail_parent['photo'];
        }
        /* breadCrumbs */
        if ($arr_breadcum) {
        ...
        $child_level = $row_detail["level"] + 1;
        $child_categories = $this->categoryRepo->GetAllItems('product', ['level' => $child_level, 'level_'.$child_level => $row_detail["id"]]);

        $sluglang = app('sluglang');
        if ($items->count() == 1) {
            $url = url($items->first()->$sluglang);
    ...
    public function CallModel($type)
    {
        ...
        } elseif (array_key_exists($type, $arr_post)) {
            $this->model = $this->postRepo;
            $this->view = 'desktop.templates.post.posts';
        } elseif (array_key_exists($type, $arr_album)) {
