...
<article class="mb-6 transition-all bg-white rounded-lg group shadow-transparent hover:shadow-lg ">
    <a {!! $fancyboxText !!} href="{{ $data["url"] }}" class="block relative mb-4  overflow-hidden
    ">
        {{-- <div class="absolute top-0 left-0 z-20 w-full h-full transition-all bg-transparent hover:bg-white hover:bg-opacity-30"></div> --}}
        <figure class="z-10 transition-all duration-300 bg-cover aspect-w-4 aspect-h-3 group-hover:scale-125" style="background-image: url({{ $config_base . $data["img"] }})"></figure>
    </a>
    <div class="px-4 pb-4">
