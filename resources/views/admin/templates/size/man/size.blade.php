...
@php
    $urlAdd = route('admin.size.edit', ['man', $type]);
    $urlDeleteAll = route('admin.size.deleteAll', ['man', $type]);
    $urlSearch = route('admin.size.show', ['man', $type]);
@endphp
@section('content')
    @csrf
                    ...
                    <table class="table table-hover">
                        <thead>
                            <tr>
                                ...
                                @if (isset($config[$type]['size_images']) && $config[$type]['size_images'])
                                <th>{{ __('Hình') }}</th>
                                @endif
                                <th>{{ __('Tiêu đề') }}</th>
                            ...
                            <tbody>
                                @foreach ($itemShow as $k => $v)
                                    @php
                                        ...
                                        $editUrl = route('admin.size.edit', ['man', $type, $v['id'] ?? 0]);
                                        $deleteUrl = route('admin.size.delete', ['man', $type, $v['id'] ?? 0]);
                                        $data = $v;
                                        $data['model'] = 'size';
                                        $data['level'] = 'man';
                                        $data['name'] = $v['tenvi'] ?? '';
