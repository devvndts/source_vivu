	...
	<div class="card-footer text-sm sticky-top">
        <div class="btn dropdown pl-0 ml-1">
          ...
          <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">            
            <a class="btn btn-sm bg-gradient-primary btn-none-css" href="{{ route('admin.danhgia.edit',['man']) }}" title="Thêm mới"><i class="fas fa-plus mr-2"></i>Thêm mới</a>
            <a class="btn btn-sm bg-gradient-danger btn-none-css" id="delete-all" data-url="{{ route('admin.danhgia.deleteAll', ['man']) }}" title="{{ __('Xóa tất cả') }}"><i class="far fa-trash-alt mr-2"></i>{{ __('Xóa tất cả') }}</a> 
          </div>
        </div>		       
    </div>
                ...
                <div class="card-header">
                    <div class="form-inline form-search align-middle mb-3">
                        <div class="input-group input-group-sm">
                            <input class="form-control form-control-navbar text-sm" type="search" id="keyword" placeholder="{{ __('Tìm kiếm') }}" aria-label="Tìm kiếm" value="{{ (isset($request->keyword))?$request->keyword:'' }}" onkeypress="doEnter(event,'keyword','{{ route('admin.danhgia.show',['man']) }}')">
                            <div class="input-group-append bg-primary rounded-right">
                                <button class="btn btn-navbar text-white" type="button" onclick="onSearch('keyword','{{ route('admin.danhgia.show',['man']) }}')">
                            ...
                            <tbody>
                                @foreach($itemShow as $k=>$v)
                                    <tr>
        								...
        								<td class="align-middle">
                                            <a class="text-danger" href="{{route('admin.danhgia.edit',['man',$v['id']])}}" title="{{$v['email']}}">{{$v['email']}}</a>
                                        </td>
                                        ...
                                        <td class="align-middle text-center dev-item-display show-checkbox">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input" data-model="danhgia" data-level="man" data-id="{{ $v['id'] }}" data-loai="hienthi" {{($v['hienthi'])?'checked':''}}>
                                                <label for="show-checkbox-{{ $v['id'] }}" class="custom-control-label"></label>
                                            </div>
                                        </td>
                                        ...
                                        <td class="dev-item-option align-middle text-center">
                                            <div class="dropdown show">
                                              ...
                                              <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                                <a class="btn btn-sm d-block btn-none-css" href="{{route('admin.danhgia.edit',['man',$v['id']])}}" title="Trả lời"><i class="fas fa-comments"></i> Trả lời</a>
