@php
    $filtered = array_filter($itemShow, function ($value) use ($categories) {
        return $value["parent_id"] ==  $categories["id"];
    })
@endphp
...
@if ($filtered)
    @php
        $level = ' |---- ' . $level;
    @endphp
    @foreach ($filtered as $itemChild)
        @include('admin.templates.category.man.child_category_level', ['categories' => $itemChild])
    @endforeach
@endif