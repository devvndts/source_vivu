				...
				<div class="list-post-newlike">
					@foreach($posts as $k=>$v)
						...
						<div class="sidebar-tab-item">
							<a href="{{$v['tenkhongdau'.$lang]}}-{{$v['id']}}" title="" class="sidebar-tab-img himg"><img src="{{Thumb::Crop(UPLOAD_POST,$v['photo'],150,100,1)}}" alt="" width="150" height="100">
								<p class="sidebar-tab-display">
									<input class="sidebar-hienthi-checkbox" id="hienthi-{{$v['id']}}" type="checkbox" {{($v['hienthi']) ? 'checked': ''}} data-model="post" data-level="man" data-id="{{$v['id']}}" data-loai="hienthi">
									<label class="ml-1" for="hienthi-{{$v['id']}}">Hiển thị</label>
								</p>
							</a>
							<div class="sidebar-tab-info">
								...
								<div class="sidebar-tab-edittool">
									<a href="{{$v['tenkhongdau'.$lang]}}-{{$v['id']}}" target="_blank"><i class="fal fa-eye mr-1"></i>Xem</a>
									<a href="{{route('account.postnews', [$v['id']])}}"><i class="fal fa-marker mr-1"></i>Sửa</a>
									<a data-id="{{$v['id']}}" class="sidebar-tab-remove"><i class="fal fa-trash-alt mr-1"></i>Xóa</a>	
								</div>								
							</div>
						</div>
					@endforeach
...
@push('js_page')
	<script>
		$('.sidebar-tab-remove').click(function(){
			var id = $(this).attr('data-id');
			confirmDialog("delete-postnews","Bài viết sẽ bị xóa vĩnh viễn, bạn thực sự muốn thực hiện điều này ?",id);
		});

		function deletePostNews(id){
			$.ajax({
				url: '{{route('account.deletePostnews')}}',
				type: "GET",
				dataType: 'html',
				data: {id:id},
		...
		$(".sidebar-hienthi-checkbox").change(function() {
		    ...
		    $.ajax({
				...
				type: "GET",
				dataType: 'html',
				data: {id:id,model:model,level:level,loai:loai},
				success: function(result){
					//location.reload();
				}
			});
